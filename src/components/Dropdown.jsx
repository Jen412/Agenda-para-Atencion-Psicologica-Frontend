...
const Dropdown = ({idCita, fechaConfirmacion}) => {
    const [isOpen, setIsOpen] = useState(false);
    const {setModal, obtenerCita} = useCitas();
    const handleClick = async ()=>{
        await obtenerCita(idCita);
        setModal(true);
    }

     return (
        <div className="relative flex flex-col items-center justify-start w-[340px] h-56 rounded-lg">
            <button 
                type="button"
                ...
                {isOpen && (
                    <div className="bg-indigo-600 absolute top-12 left-0 flex flex-col items-start rounded-lg p-2 w-full">
                        <div className="flex w-full hover:bg-indigo-800 rounded-r-lg border-l-transparent cursor-pointer hover:border-l-white border-l-4">
                            <Link className="font-semibold p-1 w-full text-left" to={`/user/reagendar-cita/${idCita}`}>Reagendar Cita</Link>
                        </div>
                        ...
                        <div className="flex w-full hover:bg-indigo-800 rounded-r-lg border-l-transparent cursor-pointer hover:border-l-white border-l-4 mt-3">
                            <input type="button" className="text-left font-semibold p-1 cursor-pointer w-full" onClick={handleClick} value="Cancelar Cita"/>
                        </div>
                        {fechaConfirmacion ==null &&(
                            <div className="flex w-full hover:bg-indigo-800 rounded-r-lg border-l-transparent cursor-pointer hover:border-l-white border-l-4 mt-3">
                                <input type="button" className="text-left font-semibold p-1 cursor-pointer w-full" onClick={()=>setModal(true)} value="Confirmar Cita"/>
                            </div>
                        )}
                        <div className="flex w-full hover:bg-indigo-800 rounded-r-lg border-l-transparent cursor-pointer hover:border-l-white border-l-4">
                            <Link className="font-semibold p-1 w-full text-left" to={`/user/procesar-cita/${idCita}`}>Procesar Cita</Link>
                        </div>
